...
edgelist=edgelists/ppi/string_yeast_ppi.tsv
embedding_dir=embeddings/ppi/
walks_dir=walks/ppi/
...
do
	...
	do
		...
		slurm_options=$(echo \
		--job-name=performEmbeddingsPPI-${dim}-${seed} \
		--time=${days}-${hrs}:00:00 \
		--mem=${mem} \
		--output=performEmbeddingsPPI-${dim}-${seed}.out \
		--error=performEmbeddingsPPI-${dim}-${seed}.err
		)
