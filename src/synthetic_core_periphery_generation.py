...
def build_stochastic_block_model(num_nodes, core_periphery_probs, connection_probs, 
	...
	random_nodes = np.arange(num_nodes)
	...
	for theta1 in np.arange(0.10, 1.0, 0.05):
		for theta2 in np.arange(0.05, theta1, 0.05):
			...
			if exp == "one_core":
				...
				connection_probs = np.array([
											[theta1, theta1, theta2, theta2, 0, theta2],
											[theta1, theta2, theta2, theta2, 0, theta2],
											[theta2, theta2, theta1, theta1, 0, theta2],
											[theta2, theta2, theta1, theta2, 0, theta2],
											[0,		 0,		 0,		 0,	     0,	0],
											[theta2, theta2, theta2, theta2, 0, theta2]])
			...
			nx.write_edgelist(g, edgelist_filename, delimiter="\t", data=["weight"])
